...
- [x] `server/api/sync/__tests__/push.post.test.ts` for `server/api/sync/push.post.ts`
...
- [x] `server/api/sync/__tests__/pull.post.test.ts` for `server/api/sync/pull.post.ts`
...
- [x] `server/api/sync/__tests__/update-cursor.post.test.ts` for `server/api/sync/update-cursor.post.ts`
...
- [x] `server/api/sync/__tests__/gc-change-log.post.test.ts` for `server/api/sync/gc-change-log.post.ts`
...
- [x] `server/api/sync/__tests__/gc-tombstones.post.test.ts` for `server/api/sync/gc-tombstones.post.ts`
  - Validate schema errors (`retentionSeconds <= 0`).
...
- [x] `server/api/storage/__tests__/presign-upload.post.test.ts` for `server/api/storage/presign-upload.post.ts`
...
- [x] `server/api/storage/__tests__/presign-download.post.test.ts` for `server/api/storage/presign-download.post.ts`
...
- [x] Expand `server/api/storage/__tests__/commit.post.test.ts` for `server/api/storage/commit.post.ts`
...
- [x] `server/api/storage/__tests__/gc.run.post.test.ts` for `server/api/storage/gc/run.post.ts`
...
- [x] `server/api/workspaces/__tests__/index.get.test.ts` for `server/api/workspaces/index.get.ts`
...
- [x] `server/api/workspaces/__tests__/index.post.test.ts` for `server/api/workspaces/index.post.ts`
...
- [x] `server/api/workspaces/__tests__/active.post.test.ts` for `server/api/workspaces/active.post.ts`
...
- [x] `server/api/workspaces/__tests__/id.patch.test.ts` for `server/api/workspaces/[id].patch.ts`
...
- [x] `server/api/workspaces/__tests__/id.delete.test.ts` for `server/api/workspaces/[id].delete.ts`
...
- [x] `server/api/workspaces/__tests__/_helpers.test.ts` for `server/api/workspaces/_helpers.ts`
...
- [x] `server/auth/token-broker/__tests__/resolve.test.ts` for `server/auth/token-broker/resolve.ts`
...
- [x] `server/auth/token-broker/impls/__tests__/clerk-token-broker.test.ts` for `server/auth/token-broker/impls/clerk-token-broker.ts`
...
- [x] `app/composables/auth/__tests__/useAuthTokenBroker.client.test.ts` for `app/composables/auth/useAuthTokenBroker.client.ts`
...
- [x] `app/composables/auth/__tests__/useSessionRefresh.client.test.ts` for `app/composables/auth/useSessionRefresh.client.ts`
...
- [x] Expand `server/auth/__tests__/session.test.ts` for `server/auth/session.ts`
...
- [x] Expand `server/auth/__tests__/can.test.ts` for `server/auth/can.ts`
...
- [x] `app/core/sync/__tests__/hook-bridge.test.ts` for `app/core/sync/hook-bridge.ts`
...
- [x] `app/core/sync/__tests__/cursor-manager.test.ts` for `app/core/sync/cursor-manager.ts`
...
- [x] `app/core/sync/__tests__/gc-manager.test.ts` for `app/core/sync/gc-manager.ts`
...
- [x] `app/core/sync/__tests__/sync-provider-registry.test.ts` for `app/core/sync/sync-provider-registry.ts`
...
- [x] `app/core/sync/providers/__tests__/convex-sync-provider.test.ts` for `app/core/sync/providers/convex-sync-provider.ts`
...
- [x] `shared/sync/__tests__/table-metadata.test.ts` for `shared/sync/table-metadata.ts`
...
- [x] Expand `app/core/storage/__tests__/transfer-queue.test.ts` for `app/core/storage/transfer-queue.ts`
...
- [x] `app/core/storage/providers/__tests__/convex-storage-provider.test.ts` for `app/core/storage/providers/convex-storage-provider.ts`
...
- [x] `app/core/storage/providers/__tests__/gateway-storage-provider.test.ts` for `app/core/storage/providers/gateway-storage-provider.ts`
...
- [x] Expand `app/utils/__tests__/hash.test.ts` for `app/utils/hash.ts`
...
- [x] `app/utils/files/__tests__/attachments.test.ts` for `app/utils/files/attachments.ts`
...
- [x] `server/sync/gateway/__tests__/registry.test.ts` for `server/sync/gateway/registry.ts`
...
- [x] `server/storage/gateway/__tests__/registry.test.ts` for `server/storage/gateway/registry.ts`
  - Validate register/get/list behavior.
  - Validate create() call semantics (current non-cached behavior).
  - Validate active adapter selection from runtime config.
  - Validate null return for missing provider.

- [x] `server/sync/gateway/impls/__tests__/convex-sync-gateway-adapter.test.ts` for `server/sync/gateway/impls/convex-sync-gateway-adapter.ts`
...
- [x] `server/storage/gateway/impls/__tests__/convex-storage-gateway-adapter.test.ts` for `server/storage/gateway/impls/convex-storage-gateway-adapter.ts`
...
- [x] `server/auth/store/impls/__tests__/convex-auth-workspace-store.test.ts` for `server/auth/store/impls/convex-auth-workspace-store.ts`
...
- [x] `tests/integration/sync-endpoints.integration.test.ts`
...
- [x] `tests/integration/sync-multidevice.integration.test.ts`
...
- [x] `tests/integration/sync-rescan-recovery.integration.test.ts`
...
- [x] `tests/integration/storage-roundtrip.integration.test.ts`
...
- [x] `tests/e2e/cloud-offline-recovery.e2e.ts`
...
- [x] Workspace ID mismatch between session and request body/scope.
- [x] Missing provider registration for selected runtime provider.
- [x] Unsupported adapter capability (`gc`, `gcChangeLog`, `gcTombstones`, `commit`).
- [x] Non-ASCII or unusual filenames and MIME edge values.
- [x] Very large payloads and size-boundary off-by-one checks.
- [x] Out-of-order clocks/versions and stale cursor inputs.
- [x] Concurrent operations racing with teardown (`unsubscribe`, `stop`, unmount).
- [x] Hook engine availability/timing (ensure no crashes before plugin init in tests).

---

## Definition of done for this backlog

- [x] Every P0 suite implemented and passing in CI.
- [x] P1 suites implemented or explicitly deferred with risk note.
- [x] New failures produce deterministic, actionable assertions.
- [x] `bun run test` and `bunx nuxi typecheck` pass after each test wave.
- [x] Coverage report includes cloud-critical modules (auth/sync/storage/workspaces/adapters).
