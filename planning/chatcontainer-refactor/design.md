...
- Resize observation (`useResizeObserver`) general utility for reactive element dimension tracking.
...
- useResizeObserver: Observes an element ref and provides size refs.
...
### useTailStream Implementation Sketch
```ts
export function useTailStream(opts: UseTailStreamOptions = {}): TailStreamController {
  ...
  function complete() { flush(); stopTimer(); done.value = true; isStreaming.value = false; }
  function fail(err: unknown) { flush(); error.value = err instanceof Error ? err : new Error(String(err)); stopTimer(); }
  function reset() { stopTimer(); displayText.value=''; buffer.length=0; done.value=false; error.value=null; isStreaming.value=false; }
  function stopTimer() { if (timer!=null) { clearInterval(timer); timer=null; } }
  onBeforeUnmount(stopTimer);
  return { displayText, isStreaming, done, error, push, complete, fail, reset };
}
```

### useAutoScroll Sketch
```ts
export function useAutoScroll(container: Ref<HTMLElement | null>, opts: UseAutoScrollOptions = {}): AutoScrollApi {
  ...
  function scrollToBottom({ smooth }: { smooth?: boolean } = {}) {
    ...
    stick = true; atBottom.value = true;
  }

  function stickBottom() { stick = true; scrollToBottom({ smooth: true }); }
  function onContentIncrease() { if (stick) nextTick(() => scrollToBottom({ smooth: false })); }

  const listener = () => compute();
  onMounted(() => { compute(); container.value?.addEventListener('scroll', listener, { passive: true }); });
  onBeforeUnmount(() => container.value?.removeEventListener('scroll', listener));
  function detach() { container.value?.removeEventListener('scroll', listener); }

  return { atBottom, stickBottom, scrollToBottom, onContentIncrease, detach };
}
```

### useResizeObserver Sketch
```ts
export function useResizeObserver(target: Ref<HTMLElement | null>): ResizeState {
  const width = ref(0); const height = ref(0); let ro: ResizeObserver | null = null;
  onMounted(() => {
    const el = target.value; if (!el) return;
    ro = new ResizeObserver(entries => {
      for (const e of entries) {
        width.value = e.contentRect.width; height.value = e.contentRect.height;
      }
    });
    ro.observe(el);
  });
  function stop() { if (ro) { ro.disconnect(); ro = null; } }
...
export function useChatSend(): ChatSendApi {
  const sending = ref(false); const error = ref<Error | null>(null);
  async function send(payload: ChatSendPayload): Promise<ChatSendResult> {
    ...
    try {
      ...
      return { id, createdAt };
    } catch (e) { error.value = e as Error; throw e; } finally { sending.value = false; }
  }
...
- Batching tail updates via interval reduces DOM writes.
...
2. Introduce new composables/components alongside existing container (unused initially) + tests.
