...
- [x] 1.1 Extend the dedicated launcher to accept `--plugin <root>` and resolve a stable project-specific profile.
...
- [x] 1.2 Add the smallest environment-loading seam needed for managed plugin startup and initialize local credentials.
...
- [x] 1.3 Supervise the profile and child processes, isolating generated Nuxt/HMR state where required.
...
- [x] 2.1 Implement one initial candidate build and the serialized source watcher.
...
- [x] 2.2 Integrate standard profile generation, source exclusions, and actionable build diagnostics.
...
- [x] 2.3 Implement the two authenticated status/artifact read routes.
...
- [x] 2.4 Add the first development page/controller using existing admission and browser canary helpers.
...
- [x] 3.2 Complete permission/setup resolution and invalidation of stale decisions.
...
- [x] 3.3 Add one-controller ownership, disconnection recovery, and state-aware retries.
...
- [x] 3.4 Adjust development admission limits and implement bounded artifact cleanup.
...
- [x] 3.5 Verify real surface replacement and fix only observed lifecycle gaps.
...
- [x] 4.1 Add the SDK `dev` command and starter `dev` script.
...
- [x] 4.2 Add host `--create` orchestration and beginner output.
...
- [x] 4.3 Connect guided local sign-in and return navigation.
      Components: C2, C5. Requirements: R2.AC2, R7.AC2, R6.AC1, R6.AC4.
      Done when: the existing admin login page's dev-only mode accepts the printed password once, obtains normal app/admin sessions through existing login APIs, resolves the workspace, and returns to Chat. Test partial login failures, expiry, safe return-target validation, and production absence; no anonymous candidate access or token-bearing URL is introduced.

- [x] 4.4 Polish accessible status and recovery affordances.
      Components: C5. Requirements: R4.AC1, R4.AC2, R4.AC3, R5.AC3.
      Done when: status is compact and announced accessibly in Chat, errors identify the next action, review/setup controls are keyboard usable, there is no success-toast flood, and the real plugin opens as a Chat workspace tab automatically. Internal candidate mechanics remain in optional details rather than the primary flow.

## 5. Verify and document the complete experience

- [x] 5.1 Add and run a named local browser harness for fresh-project development.
...
- [ ] 5.2 Complete boundary, SDK packaging, and host validation.
      ...
      Current check: affected tests, SDK build/typecheck, packed external smoke, production build, and static boundary checks pass. Host type-check still fails on the unchanged `flushProviderProgress` reference in `server/utils/background-jobs/stream-handler.ts:696`.

- [x] 5.3 Update public documentation and SDK/starter instructions.
...
- [x] 5.4 Perform final simplification and diff review.
...
- [x] 5.5 Correct and verify the developer's landing surface.
      Components: C2, C5. Requirements: R4.AC1, R5.AC1.
      Done when: the launcher signs in to `/chat`, the plugin opens as a selected Chat tab, saving the calculator source updates that pane without a page reload, normal running leaves the workspace unobstructed, errors use a small dismissible panel, and the named browser harness passes against Chat rather than an admin preview.

## Acceptance evidence

- `bun run test:e2e:plugin-development` passed against the real Chat workspace with a fresh external starter and packed-SDK restart. Sixty live saves measured 1,305 ms p95 against the 3,000 ms target. The browser run covered bad-password rejection, permission expansion, syntax-error recovery and dismissal, same-digest no-op, two tabs, storage through replacement and restart in one browser profile, cleanup retry, session expiry, and safe return navigation. A disposable calculator was also edited and used in Chrome's Chat pane; its syntax error produced a small dismissible panel and source recovery removed it.
- Affected script, core, and static-boundary Vitest suites, SDK build/typecheck, and the Nuxt production build passed. The SDK tarball includes the `dev` CLI and excludes `.or3-dev`, local profiles, browser state, and credentials.
- Host typecheck still has the unchanged `flushProviderProgress` error recorded under 5.2. No registry release or production deployment was performed.
