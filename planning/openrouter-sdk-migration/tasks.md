...
-   [x] 1.1 Add `@openrouter/sdk` to `package.json` dependencies
    ```bash
    bun add @openrouter/sdk
    ```
-   [x] 1.2 Run `bun install` and verify no dependency conflicts
-   [x] 1.3 Verify SDK is properly installed with `import { OpenRouter } from '@openrouter/sdk'`
...
-   [x] 2.1 Create `shared/openrouter/client.ts`

    -   [x] 2.1.1 Define `OpenRouterClientConfig` interface
    -   [x] 2.1.2 Implement `createOpenRouterClient()` factory function
    -   [x] 2.1.3 Implement `getRequestOptions()` helper for common headers
    -   [x] 2.1.4 Add default headers constant (`HTTP-Referer`, `X-Title`)
    -   [ ] 2.1.5 Re-export SDK types for convenience

-   [x] 2.2 Create `shared/openrouter/errors.ts`

    -   [x] 2.2.1 Define `NormalizedError` interface
    -   [x] 2.2.2 Import all SDK error classes
    -   [x] 2.2.3 Implement `normalizeSDKError()` function
        -   [x] Handle `UnauthorizedResponseError` → ERR_AUTH
        -   [x] Handle `PaymentRequiredResponseError` → ERR_CREDITS
        -   [x] Handle `ForbiddenResponseError` → ERR_FORBIDDEN
        -   [x] Handle `TooManyRequestsResponseError` → ERR_RATE_LIMIT
        -   [x] Handle `BadRequestResponseError` → ERR_BAD_REQUEST
        -   [x] Handle `NotFoundResponseError` → ERR_NOT_FOUND
        -   [x] Handle `RequestTimeoutResponseError` → ERR_TIMEOUT
        -   [x] Handle `EdgeNetworkTimeoutResponseError` → ERR_TIMEOUT
        -   [x] Handle `InternalServerResponseError` → ERR_SERVER
        -   [x] Handle `BadGatewayResponseError` → ERR_PROVIDER
        -   [x] Handle `ServiceUnavailableResponseError` → ERR_PROVIDER
        -   [x] Handle `ProviderOverloadedResponseError` → ERR_OVERLOADED
        -   [x] Handle `ChatError` → ERR_CHAT
        -   [x] Handle `AbortError` → ERR_ABORTED
        -   [x] Handle generic `Error` fallback → ERR_UNKNOWN
    -   [ ] 2.2.4 Implement `isSDKError()` type guard function

-   [x] 2.3 Create `shared/openrouter/types.ts`

    -   [x] 2.3.1 Import SDK `Model` type
    -   [x] 2.3.2 Import local `OpenRouterModel` type
    -   [x] 2.3.3 Implement `sdkModelToLocal()` mapping function
        -   [x] Map `id`, `name`, `description`, `created`
        -   [x] Map `architecture` (camelCase → snake_case)
        -   [x] Map `topProvider` → `top_provider`
...
-   [x] 5.1 Update `app/core/auth/models-service.ts`

    -   [x] 5.1.1 Add SDK and adapter imports
    -   [x] 5.1.2 Replace `fetch()` call with `client.models.list()`
    -   [x] 5.1.3 Map SDK response using `sdkModelToLocal()`
    -   [x] 5.1.4 Update error handling to use `normalizeSDKError()`
    -   [x] 5.1.5 Preserve cache logic (localStorage, TTL)
    -   [x] 5.1.6 Preserve fallback to cache on error
    -   [x] 5.1.7 Handle case where no API key is available
...
-   [x] 6.1 Update `app/core/auth/openrouter-auth.ts`

    -   [x] 6.1.1 Add SDK and adapter imports
    -   [x] 6.1.2 Replace `fetch()` call with `client.oAuth.exchangeAuthCodeForAPIKey()`
    -   [x] 6.1.3 Map SDK parameter names (camelCase)
        -   [x] `code` → `code`
        -   [x] `code_verifier` → `codeVerifier`
        -   [x] `code_challenge_method` → `codeChallengeMethod`
    -   [x] 6.1.4 Extract `key` from SDK response
    -   [x] 6.1.5 Update error handling to use `normalizeSDKError()`
    -   [x] 6.1.6 Preserve `ExchangeResult` return type
    -   [x] 6.1.7 Preserve `reportError()` integration
    -   [x] 6.1.8 Remove `fetchImpl` parameter (no longer needed)
...
-   [x] 7.1 Update `app/plugins/EditorAutocomplete/TiptapExtension.ts`

    -   [x] 7.1.1 Add SDK and adapter imports
    -   [x] 7.1.2 Replace `fetch()` call with `client.chat.send()`
    -   [x] 7.1.3 Update message format for SDK
    -   [x] 7.1.4 Pass `AbortSignal` via `getRequestOptions(signal)`
    -   [x] 7.1.5 Extract content from SDK `ChatResponse`
    -   [x] 7.1.6 Update error handling to use `normalizeSDKError()`
    -   [x] 7.1.7 Re-throw `AbortError` for existing handling
    -   [x] 7.1.8 Preserve prompt parsing logic unchanged
...
-   [x] 8.1 Update `server/api/openrouter/stream.post.ts`

    -   [ ] 8.1.1 Import `ChatGenerationParams` from SDK
    -   [ ] 8.1.2 Type request body with `ChatGenerationParams`
    -   [x] 8.1.3 Keep raw `fetch()` for streaming (SDK limitation)
    -   [x] 8.1.4 Add comment explaining streaming design decision
    -   [x] 8.1.5 Verify all existing behavior preserved

-   [x] 8.2 Update `app/utils/chat/openrouterStream.ts`

    -   [ ] 8.2.1 Import `ChatGenerationParams` from SDK
    -   [ ] 8.2.2 Add `satisfies ChatGenerationParams` to request body
    -   [x] 8.2.3 Keep raw `fetch()` for streaming (SDK limitation)
    -   [x] 8.2.4 Add comment explaining streaming design decision
    -   [x] 8.2.5 Verify all existing behavior preserved
...
-   [x] 9.1 Run all unit tests: `bun run test`
-   [x] 9.2 Verify zero test failures (Note: 1 unrelated theme test failing)
...
-   [x] 11.1 Build production bundle: `bun run build`
