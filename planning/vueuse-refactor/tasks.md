...
[ ] 1. Debounce/Throttle swaps (Req: 2, 8)
[ ] 1.1 Replace manual timers with `watchDebounced`/`useDebounceFn` in `app/composables/sidebar/useSidebarSearch.ts`; keep delays/constants.
[ ] 1.2 Swap query debounce in `app/composables/threads/useThreadSearch.ts` to `watchDebounced`.
[ ] 1.3 Replace save debounce in `app/composables/documents/useDocumentsStore.ts` with `useDebounceFn`; remove timer from state.
[ ] 1.4 Update `app/components/documents/SearchPanelRoot.vue` to use `useDebounceFn`; drop manual cleanup.
[ ] 1.5 Update delayed commits in `app/components/dashboard/ThemePage.vue` with `useDebounceFn` for sliders/settings.
[ ] 1.6 Replace prompt save debounce in `app/components/prompts/PromptEditor.vue` with `useDebounceFn`; verify autosave timing.

[ ] 2. Scroll lock composable (Req: 3, 8)
[ ] 2.1 Refactor `app/composables/core/useScrollLock.ts` to wrap VueUse `useScrollLock`, keeping `lock/unlock/isLocked` API.
[ ] 2.2 Add tests to ensure body overflow restored on dispose and parity with previous behavior.

[ ] 3. Clipboard & file flows (Req: 4, 8)
[ ] 3.1 Replace copy handlers in `app/components/chat/ChatMessage.vue` with `useClipboard`; keep UI state/notifications.
[ ] 3.2 Replace copy handlers in `app/components/chat/WorkflowChatMessage.vue` with `useClipboard`.
[ ] 3.3 Swap theme copy action in `app/components/dashboard/ThemePage.vue` to `useClipboard`.
[ ] 3.4 Refactor `app/components/chat/ChatInputDropper.vue` file input to `useFileDialog` (and `useDropZone` if drop used); preserve validation and last-model cache hook.
