...
## Task List
- [ ] Replace every `any` in the sidebar composables with safe types or guarded `unknown`.
- [ ] Add Zod validation to plugin registration (`registerPaneApp`, `registerSidebarPage`) and post meta handling.
- [ ] Refactor the global registry to use Vue reactive state (no manual `version++` counters).
- [ ] Harden activation hooks with proper error boundaries, logging, and user-friendly fallbacks.
- [ ] Debounce the Orama search index rebuild and keep it out of the hot path.
- [ ] Backfill or fix unit tests so the new behaviour is covered (validation, reactivity, debounce).
- [ ] Finish by running `bun run typecheck` and `bun test` to prove everything still compiles.
...
1. Add schemas for `PaneAppDef` and `SidebarPageDef` in their respective composables:
   ...
   const PaneAppDefSchema = z.object({
       id: z.string().min(1).regex(/^[a-z0-9-]+$/, 'lowercase id only'),
       ...
       createInitialRecord: z
           ...
           .returns(z.promise(z.union([z.object({ id: z.string() }), z.null()])))
           .optional(),
   });
   ```
...
1. In `useSidebarSearch.ts`, replace the `watch` that instantly rebuilds with something like:
   ...
   watch([threads, projects, documents], () => {
