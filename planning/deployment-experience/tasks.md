...
>
> Phase 10 release log. The first attempt qualified `0.1.68` (candidate run 35053926672, all jobs green), promoted both images, and published `@or3/cloud@0.1.68`. The tag workflow's **post-publication dashboard lifecycle verification then failed**: the previously published `0.1.67` operator hard-requires `or3Cloud.dashboardUpdateProtocol === 1`, but `0.1.68` declared protocol 2, so the old operator rejected the new release at `/check` (503). Because published versions and tags are immutable, the correction is a new patch, `0.1.69`, which declares `dashboardUpdateProtocol: 1` for the bridge (the new operator still serves the additive `/v2/*` protocol-2 endpoints and the app already falls back to protocol 1). `0.1.68` remains published and usable for host-CLI updates; `0.1.69` restores the previous-operator dashboard upgrade path. The `0.1.69` tag workflow then failed the same post-publication job **after the update reached `starting-target`**: the required `public-health` check used the host loopback URL, which is unreachable from inside the dashboard operator container (its own network namespace). The check is now caller-network-independent — a public deployment is verified through its real HTTPS origin and a local deployment from inside its own container — and `0.1.70` carries that fix.
>
> **Phase 10 status: 10.1–10.3 are complete; 10.4 is owner-gated and pending.** `@or3/cloud@0.1.70` is published from source `bd1f1310` with a green candidate and a green tag workflow (including post-publication dashboard lifecycle verification); see the release log and the 10.3 receipt. The Docker/browser lifecycle steps above are written and wired; the candidate workflow executed the image/architecture/native-addon/lifecycle checks, and the tag workflow executed the published dashboard update/rollback/persistence checks. The pre-existing `reka-ui` fixed-profile lock drift was real and is now fixed. Only a controlled production deployment with the owner's real OpenRouter flow and persistence acceptance (10.4) remains.
...
- [x] 10.1 Run proportionate final source/package verification (2–4h active work).
      ...
      Done: clean commit `bd1f1310` passed `release:prepare -- --version 0.1.70 --registry --repository` (lock contract, Cloud package, guardrail tests, docs/tarball, fixed-profile typecheck) plus `packages/or3-cloud check` (104), `test:scripts` (101), `test:release-policy` (30). The `--full`/browser/build/perf lanes are owned by the candidate workflow and Extended validation per `docs/releasing.md`, not the Cloud critical path; the candidate jobs below ran them against the exact images.

- [x] 10.2 Qualify the exact candidate and compatibility transition (2–4h active work).
      ...
      Done: candidate run [35056099562](https://github.com/Saluana/or3-chat/actions/runs/35056099562) passed at source `bd1f1310` (identity, both image builds, manifest/anonymous pulls, amd64+arm64 scans, arm64 native SQLite, digest-bound package, upgrade/rollback/restart/persistence, immutable receipt). No new jobs/matrix/builds were added.

- [x] 10.3 Publish through the existing tag workflow and independently verify artifacts (1–3h active work).
      ...
      Done: tag `v0.1.70` at `bd1f1310`; release run [35056733534](https://github.com/Saluana/or3-chat/actions/runs/35056733534) passed promote, publish-npm, and **post-publication dashboard lifecycle verification**. Independently verified: `@or3/cloud@0.1.70` integrity `sha512-7jUVcM6ASAgiNrrTafyTCmWfnfENxCSUHnwkpBe1hPDWXefSSezM8Ebnw51tTuM9NO6RZNyKTr8+gpehF03qwg==` / shasum `f80971606cb27123d80781496398da90b56eac8c`, clean-cache `npx @or3/cloud@0.1.70 --version` → `0.1.70`, and `ghcr.io/saluana/or3-chat:0.1.70` and `:0.1.70-operator` indexes each contain `linux/amd64` + `linux/arm64`. Triggering defects (protocol 2 rejection by the published 0.1.67 operator; host-loopback health check inside the operator container) were fixed in `0.1.69`/`0.1.70`; `0.1.68`/`0.1.69` remain immutable and published.
...
- [ ] 10.4 Perform a controlled exact-version managed deployment and acceptance (2–4h active work).
      ...
      **Production deployment complete; owner acceptance pending.** On 2026-09-16, `/root/apps/or3-cloud-managed-0135` (`cloud.or3.chat`) was observed healthy on `0.1.67` with 29 GB free, six authenticated backups, matching image identity, passing doctor/deep health, and no pending operation. The exact `0.1.70` dry-run reported no blockers, then `npx --yes @or3/cloud@0.1.70 update --to 0.1.70` completed without maintenance warnings. The deployed app digest is `sha256:98bf853b1f7f4e482ce705b9819c2921be648b30e9ad52980e1f12641fbe8064`; the operator digest is `sha256:bd585210d63c09d98073d19bb4c4f6ba598059fe6c91d6b8db7c8f646cd86161`. Rollback snapshot `backup-2026-09-16T04-57-50-168Z-f6fb3dd1` (source `0.1.67`) remains authenticated and preserved. Before and after one managed restart, doctor and `verify --public` passed exact digest, HTTPS root/deep-health without redirects, Basic Auth sign-in/session hydration, SQLite sync pull, filesystem storage probe, both database `quick_check`/ownership checks, and bounded log scanning. Automatic retention removed only the oldest authenticated `0.1.38` snapshot; five older authenticated snapshots, the new rollback snapshot, five legacy-unsigned archives, and the legacy adoption source remain. No production fault injection, manual state edit, or volume operation was performed. Remaining owner-only acceptance: complete a fresh real OpenRouter authorization and visually confirm an existing conversation and uploaded file; keep the rollback snapshot until those checks pass.
