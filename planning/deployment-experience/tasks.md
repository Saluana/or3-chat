...
- [ ] 10.4 Perform a controlled exact-version managed deployment and acceptance (2–4h active work).
      ...
      **Pending (owner-gated).** No production host/deployment was provided to this session, and the guardrails forbid using production for testing or cleanup without the owner at the console. When authorized, run `npx --yes @or3/cloud@0.1.70 status`, `doctor`, and `verify --read-only` first, then `npx --yes @or3/cloud@0.1.70 update --to 0.1.70` with the matching CLI, followed by `doctor`, `verify`, a managed restart, and the owner's real OpenRouter flow and conversation/file persistence checks. Keep the rollback snapshot and previous deployment until acceptance passes. Acceptance is recorded as **pending** here.
