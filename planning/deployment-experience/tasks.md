...
- [ ] 1.1 Capture focused fixtures for current and historical deployment formats (2–3h).
...
- [ ] 1.2 Define operation, diagnostic, receipt, and compatibility contracts (3–4h).
...
- [ ] 1.3 Add deterministic lifecycle fault seams to the existing test harness (2–4h).
...
- [ ] 2.1 Implement complete classified backup inventory (3–4h).
...
- [ ] 2.2 Separate retention decisions from destructive execution (2–4h).
...
- [ ] 2.3 Expose inventory and maintenance outcomes through existing backup commands (2–3h).
...
- [ ] 3.1 Make terminal state authoritative and cleanup post-commit (3–4h).
...
- [ ] 3.2 Make completion and mirror failures replay-safe (2–3h).
...
- [ ] 3.3 Ship schema-2-aware CLI/operator readers while retaining schema-1 writes (3–4h).
...
- [ ] 3.4 Establish versioned dashboard protocol negotiation (3–4h).
...
- [ ] 3.5 Prove cleanup relief against the published source and document the bridge path (3–4h).
...
- [ ] 3.6 Add essential safety regressions to the existing required jobs (1–2h).
...
- [ ] 4.1 Refactor diagnostic loading into independent observations (2–4h).
...
- [ ] 4.2 Add `verify --read-only` and explicit command mutation policy (3–4h).
...
- [ ] 4.3 Implement shared update assessment and dry-run flags (3–4h).
...
- [ ] 4.4 Move preparation ahead of the downtime boundary and revalidate on execute (3–4h).
...
- [ ] 5.1 Implement atomic migration and validated new update journals (3–4h).
...
- [ ] 5.2 Record target-ready proof after replacement and required checks (3–4h).
...
- [ ] 5.3 Implement `recover --dry-run` and `recover --finish` (3–4h).
...
- [ ] 5.4 Make destructive recovery an explicit choice (2–4h).
...
- [ ] 5.5 Reconcile operator handoff independently from application data (3–4h).
...
- [ ] 5.6 Prove recovery data boundaries and legacy behavior (3–4h).
...
- [ ] 6.1 Implement result rendering, stable codes, and redacted JSON output (2–4h).
...
- [ ] 6.2 Persist and export the latest terminal receipt (2–3h).
...
- [ ] 6.3 Surface verification gaps, maintenance findings, and phase timing (2–3h).
...
- [ ] 7.1 Connect protocol-2 preview/results to existing admin APIs (3–4h).
...
- [ ] 7.2 Update the Operations card for preview, stages, and reconnect (3–4h).
...
- [ ] 7.3 Make OpenRouter connection failure restartable without replay (2–4h).
...
- [ ] 7.4 Extend the existing browser harness for Caddy/connection regressions (3–4h).
...
- [ ] 7.5 Exercise dashboard interruption, reload, and protocol transitions (2–4h).
...
- [ ] 8.1 Add read-only repository readiness to local release preparation (3–4h).
...
- [ ] 8.2 Document check selection and retain results using existing summaries (1–2h).
...
- [ ] 8.3 Add component-specific release notes and publication status (2–3h).
...
- [ ] 8.4 Document workflow setup and capacity recovery without bypasses (1–2h).
...
- [ ] 9.1 Refresh canonical operational docs and package/provider references (2–3h).
...
- [ ] 9.2 Improve credential handoff and source-development guidance (1–3h).
...
- [ ] 9.3 Perform a simplification and traceability review (1–2h).
