...
-   [x] 13.1 Update `ChatContainer.vue` to use ThemeButton for actions
-   [x] 13.1.1 Fix hydration mismatches with explicit context props
-   [x] 13.2 Update `ModelCatalog.vue` to use ThemeButton for selections
-   [x] 13.3 Update `Dashboard.vue` to use ThemeButton for actions
-   [x] 13.4 Update `SidebarHeader.vue` to use ThemeButton for navigation
-   [x] 13.5 Update `ThemePage.vue` to use ThemeButton for theme switches
-   [x] 13.6 Update forms to use ThemeInput
-   [x] 13.7 Test all integrated components render correctly
-   [x] 13.8 Verify overrides apply in correct contexts
-   **Requirements**: 3.1

### 14. Add Manual Override Examples

-   [x] 14.1 Create example component using useThemeOverrides manually
-   [x] 14.2 Show context-specific override application
-   [x] 14.3 Show prop merging with component props
-   [x] 14.4 Add to documentation as code example
-   [x] 14.5 Test example component in all themes
-   **Requirements**: 7.1
...
## ✅ **Identifier System Implementation - COMPLETED**
...
The identifier system provides surgical precision targeting for theme components. Instead of relying on context-based resolution (which can be overridden by explicit props), identifiers act as named presets that bypass the entire override stack and provide the highest-priority appearance configuration.

### **Key Design Principles**

1. **Appearance vs Behavior Split**

    - Identifier controls: `variant`, `color`, `class`, `ui` (appearance)
    - Props control: `disabled`, `loading`, `size`, `type` (behavior)
    - This prevents state props from blocking theme appearance

2. **Precedence Hierarchy** (weakest to strongest)

    - Global rules → Context rules → State rules → Props → **Identifier (STRONGEST)**

3. **Fast Path Optimization**

    - When identifier is present, skip complex context/state resolution
    - Direct object lookup: `theme.componentOverrides.identifiers[identifier]`
    - No priority sorting, no condition filtering

4. **Developer Experience**
    - Self-documenting: `identifier="chat.send.idle"` tells you exactly what this button is
    - Flat object structure in theme.ts (easy to grep)
    - TypeScript-friendly (can be typed as string union for autocomplete)
    - Dev-mode warnings when identifier not found

### **Implementation Details**

**Files Modified:**

1. `app/components/theme/ThemeButton.vue` - Added identifier prop and fast-path resolution
2. `app/components/theme/ThemeInput.vue` - Added identifier prop and fast-path resolution
3. `app/components/theme/ThemeModal.vue` - Added identifier prop and fast-path resolution
4. `app/theme/cyberpunk/theme.ts` - Added identifiers section with chat button configs
5. `app/components/chat/ChatInputDropper.vue` - Applied identifiers to all chat buttons

**Component API:**

```vue
<!-- Dynamic identifier based on state -->
<ThemeButton
    :identifier="streaming ? 'chat.send.streaming' : 'chat.send.idle'"
    @click="handleSend"
>
  Send
</ThemeButton>

<!-- Static identifier for specific button -->
<ThemeButton identifier="chat.attach" @click="handleAttach">
...
componentOverrides: {
  identifiers: {
    'chat.send.idle': {
      variant: 'neonOutline',
      size: 'sm',
      class: 'border-pink-500 text-pink-500 shadow-[0_0_15px_rgba(255,0,255,0.5)]'
    },
    'chat.send.streaming': {
      variant: 'solid',
      color: 'error',
      size: 'sm',
      class: 'animate-pulse shadow-[0_0_20px_rgba(255,0,0,0.6)]'
    }
  }
}
```
...
const mergedProps = computed(() => {
    ...
    // Fast path: identifier lookup
    if (props.identifier) {
        const config =
            theme?.componentOverrides?.identifiers?.[props.identifier];
        if (config) {
            return {
                // Behavior from props
                disabled: props.disabled,
                loading: props.loading,
                // Appearance from identifier (WINS)
                variant: config.variant,
                color: config.color,
                ui: config.ui,
                // Size can be overridden (common use case)
    ...
    return { ...themeOverrides, ...props };
});
```

### **Usage Patterns**

**Pattern 1: State-based button appearance**

```vue
<ThemeButton :identifier="isActive ? 'sidebar.nav.active' : 'sidebar.nav.idle'">
  Navigation
</ThemeButton>
```

**Pattern 2: Context-specific precision**

```vue
<!-- Instead of relying on context detection -->
<ThemeButton context="chat">Send</ThemeButton>

<!-- Use precise identifier -->
<ThemeButton identifier="chat.send.idle">Send</ThemeButton>
```

**Pattern 3: One-off size override**

```vue
<!-- Identifier controls appearance, size prop still works -->
<ThemeButton identifier="chat.attach" size="md">
  Large Attach Button
</ThemeButton>
```
...
**Before (Props Win blocks theme):**

```vue
<ThemeButton color="info" <!-- This blocks theme override -->
  context="chat"
>
  Send
</ThemeButton>
...
**After (Identifier wins):**

```vue
<ThemeButton identifier="chat.send.idle" <!-- This ALWAYS applies -->
>
  Send
</ThemeButton>
```
...
-   **Identifier lookup**: O(1) object access
-   **Context resolution**: O(n) rule collection + sorting
-   **Cache bypass**: Identifiers skip LRU cache (always fresh)
-   **Bundle size**: +~50 lines per component (~150 bytes gzipped)

### **Testing Coverage**

-   ✅ Identifier resolution in ThemeButton
-   ✅ Identifier resolution in ThemeInput
-   ✅ Identifier resolution in ThemeModal
-   ✅ Fallback to context-based when identifier not found
-   ✅ Dev-mode warnings for missing identifiers
-   ✅ Hydration-safe rendering (no SSR mismatch)
-   ✅ Dynamic identifier switching (state changes)

### **Future Enhancements**

1. **TypeScript Union Type** (Optional)
...
2. **Visual Override Editor** (Optional)

    - Inspect component → See applied identifier
    - Edit identifier config → Live preview
    - Export to theme.ts

3. **Identifier Validation** (Optional)
    - Build-time check: all used identifiers exist in theme
    - ESLint rule to enforce identifier usage in certain contexts

### **Benefits Realized**

✅ **Solved Props Win Problem**: Explicit props no longer block theme appearance  
✅ **Maximum Precision**: Each button can have unique config without prop collision  
✅ **Simple DX**: Flat object, easy grep, self-documenting code  
✅ **Performance**: Fast O(1) lookup, no complex resolution  
✅ **Flexibility**: Context-based system still works as fallback
