...
import { readPackageZip } from './archive';
import { checkV2PackageConformance, moduleSpecifiers } from './conformance';
import {
    assertPackageRoot,
    listPackageFiles,
    materializePackTree,
...
/** Reads the manifest and the shipped runtime module graph for a package root. */
...
async function inspectPackageDirectory(
    packageRoot: string,
    options: { readonly reportedRoot?: string } = {}
    ...
    try {
        ...
        const conformance = await checkV2PackageConformance(packRoot, { mode: 'artifact' });
...
async function inspectExtractedArtifact(
    ...
    const conformance = await checkV2PackageConformance(root, { mode: 'artifact' });
    return assembleResult({
        root,
        reportedRoot,
        manifest,
...
export async function inspectV2Package(
    input: string,
    _options: { readonly repoRoot?: string } = {}
): Promise<InspectCommandResult> {
    const resolved = resolve(input);
    const stats = statSync(resolved);
    if (stats.isDirectory()) return inspectPackageDirectory(resolved);
    if (!stats.isFile()) {
    ...
    try {
        ...
        return await inspectExtractedArtifact(extractedRoot, resolved);
    } finally {
