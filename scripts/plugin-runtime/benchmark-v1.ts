...
import { createHookEngine } from '../../shared/hooks/hook-engine-core';

type BudgetClass = 'exact' | 'registry' | 'wildcard' | 'reconcile' | 'lifecycle' | 'diagnostics';
...
const budgetPath = resolve(repoRoot, 'planning/plugin-runtime-v2/benchmarks/budgets.json');
const baselinePath = resolve(repoRoot, 'planning/plugin-runtime-v2/benchmarks/milestone-0-v1.json');
...
const mode = process.argv.find((arg) => ['--report', '--check', '--record'].includes(arg)) ?? '--report';
let sink = 0;
...
function runnerMismatch(expected: RunnerFingerprint, actual: RunnerFingerprint): string[] {
...
function calibratedRepetitions(testCase: BenchmarkCase): number {
    ...
    while (repetitions < budgets.measurement.maxRepetitions) {
        ...
        repetitions = Math.min(repetitions * 2, budgets.measurement.maxRepetitions);
    }
    return repetitions;
}

function benchmark(testCase: BenchmarkCase, budgetClass: BudgetClass): BenchmarkResult {
    ...
    for (let sample = 0; sample < budgets.measurement.measurementSamples; sample++) {
        const state = testCase.create();
...
function actionCase(id: string, callbackCount: number, wildcard: boolean): BenchmarkCase {
    return {
        id,
        create() {
            const engine = createHookEngine();
            const registeredName = wildcard ? 'bench:action:*' : 'bench:action:exact';
...
function filterCase(callbackCount: number): BenchmarkCase {
    return {
        ...
        create() {
            const engine = createHookEngine();
            for (let index = 0; index < callbackCount; index++) {
                engine.addFilter('bench:filter:exact', (value) => Number(value) + 1);
            }
            return {
                run() {
...
function registryCase(recordCount: number): BenchmarkCase {
    const ids = Array.from({ length: recordCount }, (_, index) => `bench-registry-${recordCount}-${index}`);
    const globalKey = `__or3_plugin_runtime_bench_${recordCount}`;
    return {
        id: `registry.commit-dispose.${recordCount}`,
        create() {
            const registry = createRegistry<{ id: string; order: number }>(globalKey);
            for (const id of ids) registry.unregister(id);
            return {
                run() {
                    const handles = ids.map((id, order) => registry.register({ id, order }));
                    for (let index = handles.length - 1; index >= 0; index--) handles[index]!.dispose();
                },
                teardown() {
                    for (const id of ids) registry.unregister(id);
                },
...
const reconcileIds = Array.from({ length: 150 }, (_, index) => `bench-reconcile-${index}`);

function clearBenchmarkPluginInstances(): void {
    for (const plugin of listWorkspacePluginInstances()) {
        if (plugin.id.startsWith('bench-')) unregisterWorkspacePluginInstance(plugin.id);
    }
}
...
function reconcileCase(): BenchmarkCase {
    return {
        ...
        create() {
            ...
            return {
                run() {
                    const desired = new Set(useSecondSet ? reconcileIds.slice(50) : reconcileIds.slice(0, 100));
                    for (const id of active) {
                        if (!desired.has(id)) unregisterWorkspacePluginInstance(id);
                    }
                    for (const id of desired) {
                        if (!active.has(id)) registerWorkspacePluginInstance(id, 'extension', () => {});
                    }
                    active = desired;
            ...
            return {
                run() {
                    for (let cycle = 0; cycle < 1_000; cycle++) {
                        registerWorkspacePluginInstance(id, 'extension', () => {});
                        unregisterWorkspacePluginInstance(id);
                    }
                    sink++;
                },
        ...
        create() {
            ...
            return {
                run() {
                    ...
                    const timingSeries = Object.keys(engine._diagnostics.timings).length;
                    const timingSamples = Object.values(engine._diagnostics.timings).reduce(
                    ...
                    observations = {
                        ...
                        legacyEstimatedSampleBytes: timingSamples * Float64Array.BYTES_PER_ELEMENT,
                    };
                },
                observations: () => observations,
            };
...
const cases: BenchmarkCase[] = [
    ...[0, 1, 10, 100, 1_000].map((count) => actionCase(`hooks.action.exact.${count}`, count, false)),
    ...[1, 10, 100, 1_000].map(filterCase),
    ...[0, 10, 100, 1_000].map((count) => actionCase(`hooks.action.wildcard.${count}`, count, true)),
    registryCase(1),
    registryCase(100),
    reconcileCase(),
    enableDisableCase(),
...
function validateMatrix(): Map<string, BudgetClass> {
    const configured = new Map(budgets.workloads.map((workload) => [workload.id, workload.budgetClass]));
    const implemented = new Set(cases.map((testCase) => testCase.id));
    const missing = budgets.workloads.filter((workload) => !implemented.has(workload.id));
...
function checkResults(artifact: BenchmarkArtifact, baseline: BenchmarkArtifact): void {
    if (baseline.budgetSha256 !== budgetSha256) {
        throw new Error('benchmark budget changed without re-recording the reviewed Milestone 0 baseline');
    }
    const mismatches = runnerMismatch(baseline.runner, artifact.runner);
    if (mismatches.length) {
        throw new Error(`benchmark runner does not match the stored baseline:\n${mismatches.join('\n')}`);
    }
    ...
    for (const result of artifact.results) {
        ...
        const maxRatio = budgets.budgetClasses[result.budgetClass].maxMedianRatio;
        const ratio = result.medianNsPerOperation / baselineResult.medianNsPerOperation;
        if (ratio > maxRatio) {
            failures.push(`${result.id}: ${(ratio * 100).toFixed(1)}% of baseline exceeds ${(maxRatio * 100).toFixed(1)}%`);
        }
        if (JSON.stringify(result.observations ?? {}) !== JSON.stringify(baselineResult.observations ?? {})) {
            failures.push(`${result.id}: structural observations changed`);
        }
    }
    if (failures.length) throw new Error(`plugin runtime benchmark budget failed:\n${failures.join('\n')}`);
}

const configured = validateMatrix();
const actualRunner = runnerFingerprint();
const selectedRunnerMismatches = runnerMismatch(budgets.selectedRunner, actualRunner);
if (selectedRunnerMismatches.length) {
    throw new Error(`run benchmarks on the selected runner:\n${selectedRunnerMismatches.join('\n')}`);
}

const results = cases.map((testCase) => benchmark(testCase, configured.get(testCase.id)!));
...
if (mode === '--record') {
    ...
    console.log(`[plugin-runtime-benchmarks] recorded ${relative(repoRoot, baselinePath)}`);
} else if (mode === '--check') {
    const baseline = JSON.parse(readFileSync(baselinePath, 'utf8')) as BenchmarkArtifact;
    checkResults(artifact, baseline);
    console.log(`[plugin-runtime-benchmarks] ${results.length} workloads are within the Milestone 0 budgets`);
} else {
