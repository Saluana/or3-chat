...
const manifestPath = resolve(repoRoot, 'planning/plugin-runtime-v2/qualification/milestone-0.json');
...
const outputPath = shouldRecord
    ? resolve(repoRoot, `planning/plugin-runtime-v2/qualification/results/${manifest.qualificationVersion}.json`)
    : resolve(repoRoot, `.output/plugin-runtime/qualification/${manifest.qualificationVersion}.json`);
...
function evidenceDigests(): Record<string, string> {
...
function skippedCommand(command: string[]): CommandResult {
    return { command: displayCommand(command), status: 'skipped', exitCode: null, durationMs: 0 };
}
...
for (const gate of manifest.gates) {
    if (failed) {
        gateResults.push({ id: gate.id, status: 'skipped', commands: gate.commands.map(skippedCommand) });
        continue;
    }
    ...
    for (let index = 0; index < gate.commands.length; index++) {
        ...
        if (status === 'failed') {
            failed = true;
            for (const remaining of gate.commands.slice(index + 1)) commandResults.push(skippedCommand(remaining));
            break;
        }
    }
    gateResults.push({ id: gate.id, status: failed ? 'failed' : 'green', commands: commandResults });
}
