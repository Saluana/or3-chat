...
export class PluginAcquisitionService {
    ...
    async start(input: StartAcquisitionInput): Promise<StartAcquisitionResult> {
        ...
        return { ok: true, operation: await this.advance(record.operationId) };
    }

    /** Run (or resume) the pipeline to its next resting point. */
    ...
    async cancel(operationId: string): Promise<PluginAcquisitionOperation> {
        ...
        if (requested.status === 'paused' || requested.status === 'pending') {
            return await this.#finalize(requested, 'canceled', {
                code: 'canceled',
    ...
    async #download(record: PluginAcquisitionOperation): Promise<StepResult> {
        ...
        const resolved = await this.#deps.registry.resolveRelease({
            expectation: { pluginId: record.pluginId, releaseId: release.releaseId },
        });
        if (!resolved.ok) {
            return await this.#fail(
                record,
    ...
    async #recordCandidate(record: PluginAcquisitionOperation): Promise<StepResult> {
        ...
        const result = await this.#deps.services.candidates.prepare({
            ...
            loaderPreflight: ({ manifest: candidateManifest }) => {
                if (candidateManifest.trust !== 'trusted-host') {
                    return { status: 'blocked' as const, codes: ['package-trust-unsupported'] };
                }
                if (candidateManifest.runtime.client) {
                    return { status: 'blocked' as const, codes: ['trusted-host-ui-abi-unproven'] };
                }
        ...
        // A first install whose package declares setup waits for the user instead
        // of claiming an activation the plugin cannot deliver yet.
        if (!result.pointer.current && (await this.#needsSetup(record, result.stored.digest))) {
            return {
                kind: 'rest',
                operation: await this.#pause(next, 'setup-required', 'This package needs setup before it can be used.'),
            };
        }
        return { kind: 'continue', operation: next };
    }
    ...
    async #healthCheck(record: PluginAcquisitionOperation): Promise<StepResult> {
        ...
        const canary = await this.#deps.services.canary.run({
            ...
            serverDryRun: (dryRun) => serverCandidateDryRun(this.#deps.services.packages, dryRun),
            // Client activation has its own ABI release gate; a server-only
            // candidate records that rather than fabricating a browser pass.
    ...
    /** A first install pauses when the package declares setup and none is saved. */
    async #needsSetup(record: PluginAcquisitionOperation, packageDigest: string): Promise<boolean> {
        try {
            const descriptors = await loadPackageDescriptors({
                extensionsBaseDir: this.#deps.extensionsRoot ?? EXTENSIONS_BASE_DIR,
                packagePath: this.#deps.services.packages.packagePath(record.pluginId, packageDigest as `sha256-${string}`),
            });
            if (!descriptors.setup) return false;
            const saved = await this.#deps.services.settings.get(
                record.workspaceId,
                setupValuesKey(record.pluginId)
            );
            if (!saved) return true;
            const parsed = JSON.parse(saved) as unknown;
            return !parsed || typeof parsed !== 'object' || Object.keys(parsed).length === 0;
        } catch {
            // A package without readable descriptors is treated as needing setup
            // only when the descriptor exists; an error here is not a pass.
            return false;
        }
    }
