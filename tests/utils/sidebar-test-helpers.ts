...
export function createMockPanePluginApi(): PanePluginApi {
    return {
        sendMessage: vi
            .fn()
            .mockResolvedValue({ ok: true, messageId: 'message-1', threadId: 'thread-1' }),
        ...
        getPanes: vi.fn().mockReturnValue({ ok: true, panes: [], activeIndex: 0 }),
        posts: {
...
export function createMockSidebarEnvironment(
    ...
    const sections = computed<SidebarSectionGroups>(() => createMockSidebarSections())
 ...
 */
export function createMockSidebarPageControls(initialPageId: string = 'sidebar-home'): SidebarPageControls & { __activationLog: string[] } {
    ...
    return {
        ...
        set pageId(value: string | null) { pageIdRef.value = value },
 ...
 */
export function createMockSidebarPage(overrides: Partial<SidebarPageDef> = {}): SidebarPageDef {
    return {
        id: 'test-page',
        label: 'Test Page',
 ...
 */
export function registerTestPane(pageDef?: Partial<SidebarPageDef>): SidebarPageDef {
 ...
 */
export function createFakePostBuilder(overrides: Partial<any> = {}) {
    return {
 ...
 */
export function createFakePosts(count: number, baseOverrides: Partial<any> = {}): any[] {
 ...
 */
export function mountWithSidebar<T extends any>(
    component: T,
    options: any = {},
    sidebarEnvironment?: Partial<SidebarEnvironment>,
    sidebarControls?: Partial<SidebarPageControls & { __activationLog: string[] }>
) {
...
export function createMockMultiPaneApi() {
    ...
    return {
        openApp: vi.fn().mockResolvedValue(undefined),
        switchToApp: vi.fn().mockImplementation(async (appId: string, options?: any) => {
        ...
        updatePane: vi.fn(),
        // Helper for tests
        __addPane: (pane: any) => panes.value.push(pane),
...
export function setupSidebarTestEnvironment() {
    ...
    registerSidebarPage({
        ...
        component: { 
            name: 'HomePage', 
            template: '<div data-testid="sidebar-home-page">Home Page Content</div>' 
        },
    })
}
