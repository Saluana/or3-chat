...
-   `registerSidebarPostList` — high-level helper for registering post list sections.
...
**High-level helper** for quickly registering a sidebar section that displays a live list of posts by type. Perfect for custom pane apps that need sidebar integration.

### Quick Example

```ts
import { registerSidebarPostList } from '~/composables/sidebar/registerSidebarPostList';

registerSidebarPostList({
    id: 'my-plugin:tasks',
    label: 'Tasks',
    appId: 'my-task-app', // Pane app ID to open
    postType: 'task',
    ...
    renderItem: (post) => ({
        title: post.title,
        subtitle: post.meta?.status,
        icon: 'pixelarticons:note',
    }),
    reusePane: true, // Reuse existing pane if same record open
});
```
...
```ts
interface RegisterSidebarPostListOptions {
    id: string; // Unique section ID
    label: string; // Section header label
    appId: string; // Pane app ID for newPaneForApp
    postType: string; // Filter posts by this type
    icon?: string; // Section header icon (default: 'pixelarticons:list')
    placement?: 'top' | 'main' | 'bottom'; // Default: 'main'
    order?: number; // Sort order (default: 200)
    limit?: number; // Max items to show
    emptyMessage?: string; // Empty state text (default: 'No items')
    renderItem?: (post: PostData) => {
        title: string;
        subtitle?: string;
        icon?: string;
    };
    reusePane?: boolean; // Reuse existing pane (default: true)
}
```

### How it works

1. **Wraps `usePostsList`**: Automatically subscribes to live Dexie queries
2. **Renders list items**: Uses `RetroGlassBtn` for consistent styling
3. **Opens panes on click**: Calls `newPaneForApp` with the post ID
4. **Reuses existing panes**: Optional smart pane reuse when same record already open
5. **Handles states**: Loading, error, and empty states built-in

### See Also

-   `usePostsList` — underlying composable for reactive post queries
-   `usePaneApps` — register custom pane apps
-   `docs/custom-pane-apps-quickstart.md` — complete guide
