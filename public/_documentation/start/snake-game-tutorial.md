...
export class SnakeGame {
  ...
  private saveHighScore() {
    localStorage.setItem('snake-high-score', this.state.highScore.toString());
  }
...
const topScores = computed(() => {
  return scores.value
    .filter((post) => {
      ...
      return meta?.status === 'completed' && typeof meta?.score === 'number';
    })
    .sort((a, b) => {
  ...
  registerPaneApp({
    ...
    createInitialRecord: async ({ app }) => {
      try {
        ...
        if (!result.ok) {
          console.error('[snake-game] Failed to create record:', result.message);
          return { id: '' };
        }
...
- [Plugin Quickstart Guide](/start/plugin-quickstart) - Basic plugin patterns
- [useMultiPane Documentation](/composables/useMultiPane) - Pane management API
- [Posts Database Documentation](/database/posts) - Data persistence
- [Sidebar Pages Guide](/composables/useSidebarPages) - Sidebar integration
- [Hooks System](/hooks/hooks) - Event system and extensibility
