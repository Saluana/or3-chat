...
A candidate whose release asks for authority is refused until every enabled workspace has a *current* reviewed-grant record: `prepare` reports `grant-review-unreviewed` (no record) or `grant-review-stale` (the release now asks for wider access). The update check compares signed authority with existing reviews. An update covered by existing approvals needs no new consent; engine range changes remain signed compatibility metadata and are checked by the host, but do not count as new access. A site administrator can approve expanded access once for all enabled workspaces with `POST /api/admin/plugins/packages/{pluginId}/grants` and `deploymentWide: true`. The endpoint derives the requested set from the staged candidate or signed release, binds the approval to exact digests, and records it for enabled workspaces; it can be retried if a workspace store write fails. The previous selected package's approval remains available while the candidate is staged or blocked. Promotion re-checks all reviews plus workspace setup and state compatibility; a blocker leaves the current version selected and names the affected workspace and reason.
