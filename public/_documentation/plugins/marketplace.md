...
Updates are recorded candidates on the same lifecycle: the Updates view health-checks the exact reviewed bytes (including the browser canary) and only then promotes by digest. Expanded authority needs fresh workspace consent before the check can pass. Rollback, pin and uninstall keep their existing package operations, and plugin data is kept unless deletion is requested explicitly.
