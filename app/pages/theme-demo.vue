<template>
  <div class="container mx-auto p-8">
    <h1 class="text-3xl font-bold mb-8">Refined Theme System - Phase 2 Demo</h1>
    
    <div class="space-y-6">
      <!-- Basic Usage -->
      <section class="border border-gray-300 rounded-lg p-6">
        <h2 class="text-xl font-semibold mb-4">1. Basic v-theme Usage</h2>
        <p class="mb-4 text-sm text-gray-600">
          These buttons use v-theme directive without any identifier (global defaults)
        </p>
      ...
      <section class="border border-gray-300 rounded-lg p-6">
        <h2 class="text-xl font-semibold mb-4">2. Context-based Theming</h2>
        <p class="mb-4 text-sm text-gray-600">
          Context is auto-detected from DOM (this section has data-context="chat")
        </p>
        <div data-context="chat" class="space-y-4">
          ...
          <p class="text-xs text-gray-500">
            These components should receive "chat" context-specific styling from the theme
          </p>
        </div>
      </section>
      ...
      <section class="border border-gray-300 rounded-lg p-6">
        <h2 class="text-xl font-semibold mb-4">3. Identifier-based Theming</h2>
        <p class="mb-4 text-sm text-gray-600">
          Explicit identifier provides highest specificity
        </p>
        <div class="flex gap-4">
          <UButton v-theme="'chat.send'">Send Message</UButton>
          <UButton v-theme="{ identifier: 'chat.send' }">Send (Object Syntax)</UButton>
        </div>
      ...
      <!-- Programmatic Usage -->
      <section class="border border-gray-300 rounded-lg p-6">
        <h2 class="text-xl font-semibold mb-4">4. Programmatic Resolution</h2>
        <p class="mb-4 text-sm text-gray-600">
          Using useThemeOverrides composable for reactive props
        </p>
        <div class="space-y-4">
          <div>
            <label class="block text-sm mb-2">Select Context:</label>
            <select 
          ...
          <pre class="bg-gray-100 p-3 rounded text-xs overflow-auto">{{ JSON.stringify(buttonOverrides, null, 2) }}</pre>
        </div>
      </section>
      ...
      <section class="border border-gray-300 rounded-lg p-6">
        ...
        <div class="space-y-4">
          ...
          <p class="text-xs text-gray-500">
            Note: Components automatically update when theme changes (reactive)
          </p>
        </div>
      </section>
      ...
      <section class="border border-gray-300 rounded-lg p-6">
        <h2 class="text-xl font-semibold mb-4">6. Component Props Override Theme</h2>
        <p class="mb-4 text-sm text-gray-600">
      ...
      <section class="border border-gray-300 rounded-lg p-6 bg-gray-50">
        ...
        <div class="space-y-2 text-sm">
          ...
          <p><strong>Resolver Available:</strong> {{ hasResolver ? '✅ Yes' : '❌ No' }}</p>
          <p><strong>Theme Plugin:</strong> {{ hasThemePlugin ? '✅ Loaded' : '❌ Not Found' }}</p>
          <p class="text-xs text-gray-500 mt-4">
            Open browser console to see theme resolution logs (dev mode only)
          </p>
        </div>
      </section>
    </div>
...
const selectedContext = ref<'global' | 'chat' | 'sidebar' | 'dashboard'>('global');
