...
export class OverrideResolver {
    ...
    resolve(
        ...
        if (overrideContext.identifier) {
            const identifierConfig =
                this.overrides.identifiers?.[overrideContext.identifier];

            if (identifierConfig) {
                overrideProps = this.applyIdentifierOverride(
                    overrideProps,
                    identifierConfig
                );
            }
            // Note: Missing identifiers are expected - themes don't need to define all identifiers.
            // System gracefully falls back to context/state/global rules.
        }

        // Props win: start with theme overrides, then selectively apply component props
        const props = { ...overrideProps };
        for (const [key, value] of Object.entries(
            overrideContext.componentProps
        )) {
            if (value !== undefined) {
                props[key] = value;
            }
    ...
    private collectRules(
        ...
        if (
            context !== 'global' &&
            this.overrides.contexts?.[context]?.[componentType]
        ) {
    ...
    private applyIdentifierOverride(
        ...
        for (const [key, value] of Object.entries(identifierConfig)) {
            ...
            if (key === 'class') {
                merged[key] = merged[key] ? `${value} ${merged[key]}` : value;
            } else if (key === 'ui' && typeof value === 'object') {
                merged[key] = this.deepMerge(
                    (merged[key] as Record<string, unknown>) ?? {},
                    value as Record<string, unknown>
    ...
    private getCacheKey(
        ...
        const elementId =
            overrideContext.element?.id ||
            overrideContext.element?.className ||
            'no-element';
