<template>
    <section class="h-full min-h-0 overflow-y-auto bg-[var(--md-surface)] px-4 pb-8 pt-14 md:px-6" aria-label="Activity detail">
        ...
        <div v-else-if="detail" class="mx-auto max-w-4xl space-y-4">
            <header class="rounded-[var(--md-border-radius)] border border-[var(--md-outline-variant)] bg-[var(--md-surface-container)] p-4">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <p v-if="detail.summary" class="mt-1 text-sm text-[var(--md-on-surface-variant)]">
                            {{ detail.summary }}
                        </p>
                    </div>
                    <div v-if="detail.actions.length" class="flex flex-wrap gap-2">
                        <UButton
                            ...
                            :key="action"
                            size="sm"
                            :color="action === 'deny' || action === 'cancel' ? 'error' : 'primary'"
            ...
            <section v-if="detail.approvals?.length" class="space-y-2">
                ...
                >
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <UBadge color="warning" variant="soft">{{ approval.status }}</UBadge>
                    </div>
                    <p v-if="approval.description" class="mt-1 text-sm text-[var(--md-on-surface-variant)]">
            ...
            <section v-if="detail.artifacts?.length" class="space-y-2">
                ...
                <div class="grid gap-2 sm:grid-cols-2">
                    ...
                    >
                        ...
                        <span class="ml-2 text-xs text-[var(--md-outline)]">{{ artifact.kind }}</span>
                    </a>
                </div>
            </section>
            ...
            <section v-if="detail.output" class="space-y-2">
                ...
                <pre class="max-h-96 overflow-auto whitespace-pre-wrap rounded-[var(--md-border-radius)] bg-[var(--md-surface-container)] p-3 text-sm">{{ detail.output }}</pre>
            </section>

            <section class="space-y-2">
                <h2 class="text-sm font-semibold">Timeline</h2>
                <div v-if="!timelineEvents.length" class="rounded-[var(--md-border-radius)] border border-dashed border-[var(--md-outline-variant)] p-6 text-center text-sm text-[var(--md-on-surface-variant)]">
                ...
                <ol v-else class="space-y-2">
                    ...
                    >
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <time class="text-xs text-[var(--md-outline)]" :datetime="event.occurredAt">
                                {{ new Date(event.occurredAt).toLocaleString() }}
                            </time>
                        </div>
                        <p class="mt-1 whitespace-pre-wrap break-words text-sm text-[var(--md-on-surface-variant)]">
                            {{ eventText(event) }}
                        </p>
                    </li>
                </ol>
...
async function load() {
    ...
    loading.value = true;
    const result = await registry.getRun(
        refValue.sourceId,
        refValue.runId
    );
    if (!result.ok) {
...
function eventText(event: ActivityEvent): string {
    ...
    for (const key of ['text', 'message', 'prompt', 'error', 'output', 'title']) {
