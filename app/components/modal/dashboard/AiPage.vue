<template>
    <div class="px-4 py-4 space-y-12 text-sm">
        ...
        <section
            class="section-card space-y-3"
            role="group"
            aria-labelledby="ai-section-generation"
        >
            <h2
                id="ai-section-generation"
                class="font-heading text-base uppercase tracking-wide group-heading"
            >
                Generation Defaults
            </h2>
            <div class="flex items-center gap-4">
                <label class="w-40">Temperature</label>
                <input
                    type="range"
                    min="0"
                    max="2"
            ...
            <div class="flex items-center gap-4">
                <label class="w-40">Max tokens</label>
                <input
                    type="text"
                    inputmode="numeric"
                    pattern="[0-9]*"
                    ...
                    aria-describedby="ai-max-help"
                    placeholder="auto"
                />
                <span id="ai-max-help" class="text-xs opacity-70"
            ...
            >
                <input
                    ...
                    @change="set({ jsonMode: !settings.jsonMode })"
                />
                <span class="text-xs"
                    >Prefer JSON responses (if supported)</span
                >
            </label>
        </section>
        ...
        <section
            class="section-card space-y-3"
            role="group"
            aria-labelledby="ai-section-tool-policy"
        >
            <h2
                id="ai-section-tool-policy"
                class="font-heading text-base uppercase tracking-wide group-heading"
            >
                Tool Use Policy
            </h2>
            <div class="flex gap-2 items-center">
                <UButton
                    ...
                    variant="basic"
                    class="retro-chip"
                    :class="settings.toolPolicy === 'allow' ? 'active' : ''"
                    :aria-pressed="settings.toolPolicy === 'allow'"
                    :disabled="settings.toolPolicy === 'allow'"
                    @click="set({ toolPolicy: 'allow' })"
                    >Allow</UButton
                >
                <UButton
                    size="sm"
                    variant="basic"
                    class="retro-chip"
                    :class="settings.toolPolicy === 'disallow' ? 'active' : ''"
                    :aria-pressed="settings.toolPolicy === 'disallow'"
                    :disabled="settings.toolPolicy === 'disallow'"
                    @click="set({ toolPolicy: 'disallow' })"
                    >Disallow</UButton
                >
                <UButton
                    size="sm"
                    variant="basic"
                    class="retro-chip"
                    :class="settings.toolPolicy === 'ask' ? 'active' : ''"
                    :aria-pressed="settings.toolPolicy === 'ask'"
                    :disabled="settings.toolPolicy === 'ask'"
                    @click="set({ toolPolicy: 'ask' })"
                    >Ask</UButton
                >
            </div>
        </section>
