<template>
    <div class="theme-selector">
        ...
        <div
            class="current-theme mb-6 p-4 border-2 border-(--app-theme-current-border) rounded-md bg-(--md-surface) retro-shadow"
        >
            <h4 class="font-bold mb-2">Current Theme</h4>
            <div class="flex items-center gap-2">
                <div class="w-4 h-4 rounded-full bg-(--md-primary)"></div>
        ...
        <div class="theme-options">
            <div
                ...
                :key="theme.name"
                class="theme-option mb-4 p-4 border-2 rounded-md cursor-pointer transition-all border-(--app-theme-option-border) bg-(--app-theme-option-bg) retro-shadow"
                :class="{
                    'border-(--app-theme-option-active-border) bg-(--app-theme-option-active-bg) hover:border-(--app-theme-option-active-border)':
                        theme.name === activeTheme,
                    'hover:border-(--app-theme-option-hover-border)':
                        theme.name !== activeTheme,
                    'opacity-50 cursor-not-allowed':
                        isLoading && theme.name === activeTheme,
                }"
            ...
            >
                    ...
                    <div class="flex items-center gap-3">
                        <div class="theme-preview">
                            <div class="flex gap-1">
                                <div
                                    v-if="theme.hasLight"
                                    class="w-3 h-3 rounded-full border bg-(--app-theme-preview-light) border-(--app-theme-preview-light-border)"
                                    title="Light variant"
                                ></div>
                                <div
                                    v-if="theme.hasDark"
                                    class="w-3 h-3 rounded-full border bg-(--app-theme-preview-dark) border-(--app-theme-preview-dark-border)"
                                    title="Dark variant"
                                ></div>
                                <div
                                    v-if="theme.variants.includes('light-hc')"
                                    class="w-3 h-3 rounded-full border-2 bg-(--app-theme-preview-hc-light) border-(--app-theme-preview-hc-light-border)"
                                    title="Light high-contrast variant"
                                ></div>
                                <div
                                    v-if="theme.variants.includes('dark-hc')"
                                    class="w-3 h-3 rounded-full border-2 bg-(--app-theme-preview-hc-dark) border-(--app-theme-preview-hc-dark-border)"
                        ...
                        <div>
                            ...
                            <div class="flex gap-1 mt-1">
                                <span
                                    v-for="variant in theme.variants.slice(
                                        0,
                                        3
                                    )"
                                    :key="variant"
                                    class="text-xs px-1 py-0.5 bg-(--app-theme-badge-bg) rounded border border-(--app-theme-badge-border) text-(--md-on-surface)"
                                >
                                    {{ variant }}
                                </span>
                                <span
                ...
                <!-- Theme Status -->
                <div
                    v-if="validationCache[theme.name]?.errors?.length"
                    class="mt-3 text-xs text-(--md-error)"
                >
                    <div class="font-bold">Errors:</div>
                    <ul class="list-disc list-inside">
                        <li
                            v-for="error in validationCache[theme.name]
                                ?.errors || []"
                            :key="error.file"
                        >
                            {{ error.message }}
                ...
                <div
                    v-if="validationCache[theme.name]?.warnings?.length"
                    class="mt-3 text-xs text-(--md-warning)"
                >
                    <div class="font-bold">Warnings:</div>
                    <ul class="list-disc list-inside">
                        <li
                            v-for="warning in validationCache[theme.name]
                                ?.warnings || []"
                            :key="warning.file"
                        >
                            {{ warning.message }}
        ...
        <div v-if="availableThemes.length === 0" class="text-center py-8">
            ...
            <p class="text-xs opacity-50">
                Check the theme directory for available themes.
            </p>
        </div>
        ...
        <div
            ...
            class="mt-6 p-4 border-2 border-(--md-warning) rounded-md retro-shadow bg-(--app-theme-option-bg)"
        >
            <h4 class="font-bold mb-2 text-(--md-warning)">
                Theme System Status
            </h4>
            <div v-if="errors.length" class="mb-2">
                <div class="text-sm font-bold text-(--md-error)">
            ...
            <div v-if="warnings.length">
                <div class="text-sm font-bold text-(--md-warning)">
                    Warnings ({{ warnings.length }}):
                </div>
                <ul class="text-xs list-disc list-inside">
                    <li v-for="warning in warnings" :key="warning.file">
                        {{ warning.message }}
                    </li>
                </ul>
            </div>
...
const validationCache = ref<
    Record<string, { errors: ThemeError[]; warnings: ThemeWarning[] }>
>({});
...
const currentThemeManifest = computed(() => {
    return availableThemes.value.find(
        (t: ThemeManifest) => t.name === activeTheme.value
    );
});
...
watch(
    () => $theme.availableThemes.value,
    (newThemes: ThemeManifest[]) => {
...
watch(
    () => $theme.errors.value,
    (newErrors: ThemeError[]) => {
        errors.value = newErrors;
    }
);

watch(
    () => $theme.warnings.value,
    (newWarnings: ThemeWarning[]) => {
