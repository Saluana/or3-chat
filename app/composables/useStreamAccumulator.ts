...
const _raf: typeof requestAnimationFrame | ((cb: FrameRequestCallback) => number) =
  typeof requestAnimationFrame !== 'undefined'
    ? requestAnimationFrame
    : (cb: FrameRequestCallback) => setTimeout(() => cb(performance.now()), 0) as any;
const _caf: typeof cancelAnimationFrame | ((id: number) => void) =
  typeof cancelAnimationFrame !== 'undefined' ? cancelAnimationFrame : (id: number) => clearTimeout(id);
...
export function createStreamAccumulator(): StreamAccumulatorApi {
  ...
  function append(delta: string, { kind }: { kind: AppendKind }) {
    if (_finalized) {
      if (import.meta.dev) console.warn('[stream] append after finalize ignored');
      return;
    }
    if (!delta) {
    ...
    if (kind === 'reasoning') pendingReasoning.push(delta); else pendingMain.push(delta);
    schedule();
  }
  ...
  function finalize(opts?: { error?: Error; aborted?: boolean }) {
    ...
    if (pendingMain.length || pendingReasoning.length) flush(); else state.version++;
