<template>
  ...
  >
    ...
    <div class="flex items-center gap-2 px-3 pt-3 pb-2">
      ...
      />
      <UBadge v-if="task.label && task.label !== 'uncategorized'" color="neutral" variant="soft" size="sm" class="shrink-0">{{ task.label }}</UBadge>
    ...
    <div class="flex items-center gap-2 px-3 pb-3 border-b-[length:var(--md-border-width)] border-b-[color:var(--md-border-color)]">
      <!-- Due date: button triggers hidden native input -->
      <button
        class="flex items-center gap-1.5 h-[32px] px-2 text-xs border-[length:var(--md-border-width)] border-[color:var(--md-border-color)] rounded-[var(--md-border-radius)] bg-[var(--md-surface)] hover:bg-[var(--md-surface-hover)] transition-colors text-[var(--md-on-surface)] opacity-70 hover:opacity-100 relative overflow-hidden"
      ...
      <UButton size="sm" variant="outline" leading-icon="i-lucide-sparkles" @click="$emit('breakdown', task.id)">
        Break down
      </UButton>
    </div>
    ...
    <div class="flex gap-1.5 px-3 py-2" :class="task.subtasks.length ? 'border-t-[length:var(--md-border-width)] border-t-[color:var(--md-border-color)]' : ''">
      ...
      <UButton size="sm" variant="outline" @click="emitCreateSubtask">Add</UButton>
    </div>
  </div>
</template>
